...
def handle_request_error(response: requests.Response) -> None:
    raise Exception(
        f"request fail!. status code: {response.status_code}, detail: {response.text}")
    ...
    if response.ok:
        ...
        for user_info in users_info:
            answer[user_info["userId"]] = {
                "guest_page_ids": [] if user_info.get("guestPageIds") is None else user_info.get("guestPageIds")}
...
def detect_authority(user_info: Dict) -> Dict:
    d = copy.deepcopy(user_info)
    user_uids = d.keys()
    for user_uid in user_uids:
        d[user_uid]["authority"] = "unknown"

        page_ids = d[user_uid]["guest_page_ids"]
        ...
        if lubycon_flag:
            d[user_uid]["authority"] = "lubycon"
            continue

        if mentor_flag:
            d[user_uid]["authority"] = "mentor"
            continue

        if mentee_flag:
            d[user_uid]["authority"] = "mentee"
            continue
